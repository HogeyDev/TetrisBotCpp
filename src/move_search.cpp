...
std::vector<Move> getPossibleMoves(Game* currentGame, std::vector<SearchState*>& potentialTuckSpinStates) {
    ...
    int rotationEnd = pieceName == 'O' || pieceName == 'I' || pieceName == 'S' || pieceName == 'Z' ? 0 : 2;

    int xStart = pieceName == 'O' ? -4 : -5;
    int xEnd = 4;

    for (int xOffset = xStart; xOffset <= xEnd; xOffset++) {
        for (int rotationState = rotationStart; rotationState <= rotationEnd; rotationState++) {
            moveList.push_back({xOffset, rotationState});
            std::string inputTimeline = generateInputTimeline(tapTimeline, {xOffset, rotationState});
            Game* gameCopy = currentGame->clone();
            for (unsigned int i = 0; i < inputTimeline.size(); i++) {
                gameCopy->tick(inputTimeline.at(i));
            }
			
            potentialTuckSpinStates.push_back(new SearchState(gameCopy, inputTimeline));
        }
    }
    return moveList;
}

std::string getBestMove(Game* currentGame) { return getBestMove(currentGame, globalSearchDepth); }
std::string getBestMove(Game* currentGame, int searchDepth) {
    std::vector<SearchState*> potentialTuckSpinStates;
    std::vector<Move> generatedMoveList = getPossibleMoves(currentGame, potentialTuckSpinStates);
    std::vector<std::string> spinsAndTucks =
        searchForTucksAndSpins(currentGame, generatedMoveList, potentialTuckSpinStates);
    ...
    for (Move move : generatedMoveList) {
        unsortedTimelineList.push_back(generateInputTimeline(tapTimeline, move));
    }
    unsortedTimelineList.insert(unsortedTimelineList.end(), spinsAndTucks.begin(), spinsAndTucks.end());
    ...
    for (std::string timeline : unsortedTimelineList) {
        ...
        if (searchDepth > 1) {
            std::string bestSecondaryMove = getBestMove(gameCopy, searchDepth - 1);
            gameCopy->simulatePiece(bestSecondaryMove);
        }

        int currentEvaluation = evaluateGame(gameCopy);
        int insertionIndex = findEvaluationListIndex(evaluationList, currentEvaluation);

        // exit(0);

        timelineList.insert(timelineList.begin() + insertionIndex, timeline);
        evaluationList.insert(evaluationList.begin() + insertionIndex, currentEvaluation);
    }

    return timelineList[0];
}

int findEvaluationListIndex(std::vector<int> evaluations, int currentEvaluation) {
...
int evaluateGame(Game* game) {
    ...
    for (int x = 0; x < 10; x++) {
        ...
        for (int y = 19; y >= 0; y--) {
            ...
            if (game->board->getMinoXY(x, y) == 0) {
                if ((game->board->getMinoXY(x - 1, y) || game->board->getMinoXY(x + 1, y)) &&
    ...
    linesCleared =
        (game->linesLastCleared == 4 ? -1200 * game->level : (std::pow(game->linesLastCleared, 2) + 4) * 200);
    bumpiness = bumpiness * 3;
    totalHoleCount = totalHoleCount * (aboveScareHeight ? 300 + totalHoleCount * 20 : 1000);
    minosAboveHole = minosAboveHole * (aboveScareHeight ? 300 + minosAboveHole * 20 : 1000);
...
std::string generateInputTimeline(std::string frameTimeline, Move move) {
    ...
    while ((rotationsRemaining + movementsRemaining) > 0) {
        ...
        char character = '.';
        if (realFrameTimeline[i] == 'X' && (rotationsRemaining + movementsRemaining) > 0) {
...
std::vector<std::string> searchForTucksAndSpins(Game* game, std::vector<Move> baseMoveList,
                                                std::vector<SearchState*>& potentialTuckSpinStates) {
    ...
    for (unsigned int i = 0; i < baseMoveList.size(); i++) {
        ...
        std::string inputTimeline = generateInputTimeline(tapTimeline, baseMoveList[i]);
        ...
        std::vector<std::string> tuckSpinInputs = {"", "", "EI", "", "EFIG"};
        ...
        while (oldPieceTotal == gameCopy->totalPieces) {
            ...
            if (oldY != gameCopy->activePiece->y) {
                ...
                std::string possibleTuckSpinInputs = tuckSpinInputs[numOrientations];
                std::string possibleInputs = possibleTuckInputs + possibleSpinInputs + possibleTuckSpinInputs;

                for (unsigned int input = 0; input < possibleInputs.size(); input++) {
                    Game* gameCopyCopy = gameCopy->clone();
                    if (gameCopyCopy->tick(possibleInputs[input])) {
                        if (isNewState(allSeenGameStates, gameCopyCopy)) {
                            newInputTimelines.push_back(inputTimeline + possibleInputs[input]);
