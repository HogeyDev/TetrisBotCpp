...
std::vector<Move> getPossibleMoves(Game *currentGame,
  ...
  int rotationEnd = pieceName == 'O' || pieceName == 'I' || pieceName == 'S' ||
                            pieceName == 'Z'
                        ? 0
                        : 2;
...
std::string getBestMove(Game *currentGame, int searchDepth) {
  ...
  unsortedTimelineList.insert(unsortedTimelineList.end(), spinsAndTucks.begin(),
                              spinsAndTucks.end());
...
int evaluateGame(Game *game) {
  ...
  for (int x = 0; x < 10; x++) {
    ...
    boardHeight += (columnHeight >= aboveScareHeight ? columnHeight : 0);
  	if (x == wellColumn) boardHeight += 20 + columnHeight * columnHeight;
  }
  ...
  linesCleared =
      (game->linesLastCleared == 4 ? -1200 * game->level
                                   : (std::pow(game->linesLastCleared, 2) + 4) * 200);
  bumpiness = bumpiness * 3;
  totalHoleCount = totalHoleCount * (aboveScareHeight ? 300 : 1000);
  minosAboveHole = minosAboveHole * (aboveScareHeight ? 300 : 1000);
  ...
  for (unsigned int i = 0; i < baseMoveList.size(); i++) {
    ...
    while (oldPieceTotal == gameCopy->totalPieces) {
      ...
      if (oldY != gameCopy->activePiece->y) {
        ...
        // for (unsigned int input = 0; input < possibleSpinInputs.size(); input++) {
        //   Game *gameCopyCopy = gameCopy->clone();
        //   if (gameCopyCopy->tick(possibleSpinInputs[input])) {
        //     if (isNewState(allSeenGameStates, gameCopyCopy)) {
        //       tucks.push_back(inputTimeline + possibleSpinInputs[input]);
        //       allSeenGameStates.push_back(gameCopyCopy);
        //     }
        //   }
        // }
        // for (unsigned int input = 0; input < possibleTuckSpinInputs.size(); input++) {
        //   Game *gameCopyCopy = gameCopy->clone();
        //   if (gameCopyCopy->tick(possibleTuckSpinInputs[input])) {
        //     if (isNewState(allSeenGameStates, gameCopyCopy)) {
        //       tucks.push_back(inputTimeline + possibleTuckSpinInputs[input]);
        //       allSeenGameStates.push_back(gameCopyCopy);
        //     }
        //   }
        // }
